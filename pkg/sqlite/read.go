...
func (q *QueryBuilder) BuildChecked() (string, []any, error) {
	table, err := sanitizeIdentifier(q.table)
	if err != nil {
		return "", nil, err
	}
	...
	cols := "*"
	if len(q.columns) > 0 {
		sanitizedColumns, err := sanitizeIdentifierList(q.columns, sanitizeColumn)
		if err != nil {
			return "", nil, err
		}
		cols = joinStrings(sanitizedColumns)
	}
	...
	query := fmt.Sprintf("SELECT %s%s FROM %s", distinct, cols, table)
	...
	if len(q.orderBy) > 0 {
		sanitizedOrderBy, err := sanitizeIdentifierList(q.orderBy, sanitizeOrderByClause)
		if err != nil {
			return "", nil, err
		}
		query += " ORDER BY " + joinStrings(sanitizedOrderBy)
	}
	...
	return query, q.whereArgs, nil
}

func (q *QueryBuilder) Build() (string, []any) {
	query, args, err := q.BuildChecked()
	if err != nil {
		return "", q.whereArgs
	}
	return query, args
