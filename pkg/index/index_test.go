...
func TestSearchWithMetadataFilter(t *testing.T) {
	...
	items := []IndexItem{
		...
		{ID: 2, Vector: []float32{0.1, 0.2, 0.3, 0.4}, Metadata: map[string]string{"category": "chat"}},
		{ID: 3, Vector: []float32{0.9, 0.8, 0.7, 0.6}, Metadata: map[string]string{"category": "docs"}},
	}

	if _, err := im.Index(ctx, "filtered_search_index", items, nil); err != nil {
		t.Fatalf("index: %v", err)
	}

	results, err := im.SearchWithFilter(ctx, "filtered_search_index", []float32{0.1, 0.2, 0.3, 0.4}, 10, map[string]string{"category": "docs"})
	if err != nil {
	...
	for _, result := range results {
		if result.Metadata["category"] != "docs" {
			t.Fatalf("expected category docs, got %#v", result.Metadata)
		}
	}
}

func TestSearchByTextWithMetadataFilter(t *testing.T) {
	im, _ := setupIndexManager(t)
	ctx := context.Background()

	im.Create(ctx, Config{
		Name:       "text_filtered_search_index",
		Dimensions: 4,
		Metadata:   []string{"category"},
	})

	items := []IndexItem{
		{ID: 1, Vector: []float32{1, 1, 1, 1}, Metadata: map[string]string{"category": "docs"}},
		{ID: 2, Vector: []float32{1, 1, 1, 1}, Metadata: map[string]string{"category": "chat"}},
	}

	if _, err := im.Index(ctx, "text_filtered_search_index", items, nil); err != nil {
		t.Fatalf("index: %v", err)
	}

	results, err := im.SearchByTextWithFilter(ctx, "text_filtered_search_index", "four", 10, map[string]string{"category": "docs"})
	if err != nil {
		t.Fatalf("search by text with filter: %v", err)
	}

	if len(results) != 1 {
		t.Fatalf("expected 1 filtered text result, got %d", len(results))
	}
	if results[0].Metadata["category"] != "docs" {
		t.Fatalf("expected category docs, got %#v", results[0].Metadata)
	}
}
