...
func TestVecStoreSearchWithMetadataFilter(t *testing.T) {
	...
	vs.Insert(ctx, 1, []float32{0.1, 0.2, 0.3, 0.4}, map[string]string{"category": "docs", "source": "kb"})
	vs.Insert(ctx, 2, []float32{0.1, 0.2, 0.3, 0.4}, map[string]string{"category": "chat", "source": "thread"})
	vs.Insert(ctx, 3, []float32{0.9, 0.8, 0.7, 0.6}, map[string]string{"category": "docs", "source": "archive"})

	results, err := vs.SearchWithFilter(ctx, []float32{0.1, 0.2, 0.3, 0.4}, 10, 0, map[string]string{"category": "docs"})
	if err != nil {
	...
	if results[0].RowID != 1 {
		t.Fatalf("expected closest filtered result rowid 1, got %d", results[0].RowID)
	}
	for _, result := range results {
		if result.Metadata["category"] != "docs" {
			t.Fatalf("expected filtered category docs, got %#v", result.Metadata)
		}
	}
}

func TestVecStoreSearchWithFilterRejectsUnknownMetadataField(t *testing.T) {
	vs := setupVecStore(t)
	ctx := context.Background()

	if _, err := vs.SearchWithFilter(ctx, []float32{0.1, 0.2, 0.3, 0.4}, 10, 0, map[string]string{"unknown": "value"}); err == nil {
		t.Fatal("expected unknown metadata field to fail")
	}
}
