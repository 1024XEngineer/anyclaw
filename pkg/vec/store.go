...
func (vs *VecStore) SearchWithFilter(ctx context.Context, queryVector []float32, limit int, threshold float64, metadataFilter map[string]string) ([]VecSearchResult, error) {
	...
	selectCols := "SELECT matches.rowid, matches.distance"
	for _, meta := range vs.metadata {
		selectCols += fmt.Sprintf(", docs.%s", meta)
	}

	query := fmt.Sprintf(
		"WITH matches AS (SELECT rowid, distance FROM %s WHERE vector MATCH ? AND k = ?) %s FROM matches JOIN %s AS docs ON docs.rowid = matches.rowid",
		vs.tableName,
		selectCols,
		vs.tableName,
	)
	...
	conditions := make([]string, 0, len(metadataFilter)+1)

	filterClause, filterArgs, err := vs.buildMetadataFilter("docs", metadataFilter)
	if err != nil {
		return nil, err
	}
	if filterClause != "" {
	...
	if threshold > 0 {
		conditions = append(conditions, "matches.distance <= ?")
		args = append(args, threshold)
	}
	if len(conditions) > 0 {
		query += " WHERE " + strings.Join(conditions, " AND ")
	}
	query += " ORDER BY matches.distance"
...
func (vs *VecStore) buildMetadataFilter(alias string, metadataFilter map[string]string) (string, []any, error) {
	if len(metadataFilter) == 0 {
		return "", nil, nil
	}

	allowed := make(map[string]struct{}, len(vs.metadata))
	for _, key := range vs.metadata {
		allowed[key] = struct{}{}
	}
	...
	for _, key := range keys {
		if _, ok := allowed[key]; !ok {
			return "", nil, fmt.Errorf("metadata filter field %q is not configured", key)
		}
		column := key
		if alias != "" {
			column = alias + "." + key
		...
		args = append(args, metadataFilter[key])
	}
