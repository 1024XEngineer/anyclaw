...
func (c *Client) Execute(ctx context.Context, args []string) (string, error) {
	...
	switch cmd {
	case "notebook":
		return c.notebook(subArgs)
	case "source":
		return c.source(subArgs)
	case "chat":
		return c.chat(subArgs)
	case "download":
		return c.download(subArgs)
	case "help":
		return c.help()
	default:
...
func (c *Client) notebook(args []string) (string, error) {
	if len(args) == 0 {
...
func (c *Client) source(args []string) (string, error) {
	if len(args) < 2 {
	...
	req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, c.baseURL+"/notebooks/"+notebookID+"/sources", bytes.NewReader(jsonBody))
	if err != nil {
		return "", err
	}
	req.Header.Set("Authorization", "Bearer "+c.apiKey)
...
func (c *Client) chat(args []string) (string, error) {
	...
	req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, c.baseURL+"/notebooks/"+notebookID+"/chat", bytes.NewReader(jsonBody))
	if err != nil {
		return "", err
	}
	req.Header.Set("Authorization", "Bearer "+c.apiKey)
...
func (c *Client) download(args []string) (string, error) {
	if len(args) < 2 {
	...
	req, err := http.NewRequestWithContext(context.Background(), http.MethodGet, c.baseURL+"/notebooks/"+notebookID+"/export", nil)
	if err != nil {
		return "", err
	}
	req.Header.Set("Authorization", "Bearer "+c.apiKey)
