...
function normalizeArtifact(artifact: MarketArtifactDetail): MarketDirectoryEntry {
  ...
  return {
    chips,
    finalScore: artifact.final_score,
    footnote: artifact.install_hint || metadata.working_dir || metadata.entrypoint || artifact.source_id || "",
    freshnessScore: artifact.freshness_score,
    id: artifact.id,
    kind: artifact.kind,
    lexicalScore: artifact.lexical_score,
    ...
    riskPenalty: artifact.risk_penalty,
...
async function loadMarketArtifacts(kind: MarketKind, source: MarketSource, query: string, filters: MarketFilters): Promise<MarketArtifactsQueryResult> {
  ...
  const items = (payload.data?.items ?? []).map(localizeArtifactMetadata);
  const artifactLookup = Object.fromEntries(items.map((item) => [item.id, item])) as Record<string, MarketArtifactDetail>;
  return {
    artifactLookup,
...
export function useMarketDirectory() {
  ...
  return {
    actionError,
    artifactLookup: artifactsQuery.data?.artifactLookup ?? {},
