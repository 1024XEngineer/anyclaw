...
> 本文档描述了 AnyClaw 的核心 Agent 架构，借鉴了 pi-mono 和 openclaw 的设计模式。

## 执行摘要

AnyClaw 采用模块化的 Agent 架构，核心组件包括：
...
**注意**：AnyClaw 与 OpenClaw 都采用双循环架构，但恢复逻辑的放置位置不同，详见 [与 OpenClaw 的对比](#与-openclaw-循环架构的对比)。
