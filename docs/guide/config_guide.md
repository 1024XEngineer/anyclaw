...
This guide covers configuring anyclaw, including new features like multi-provider failover and WebSocket gateway.
...
```json
{
  "gateway": {
    "websocket": {
      "enable_tls": true,
      "cert_file": "/etc/ssl/certs/anyclaw.crt",
      "key_file": "/etc/ssl/private/anyclaw.key"
    }
  }
}
```
...
```bash
export ANYCLAW_OPENAI_API_KEY="sk-..."
export ANYCLAW_TELEGRAM_TOKEN="123456789:ABC..."
export ANYCLAW_WS_AUTH_TOKEN="secure-token"
```

### Configuration File Locations

anyclaw searches for config in this order:
1. `~/.anyclaw/config.json` (highest priority)
