...
AnyClaw (🐾 狗爪) 是一个功能强大的 AI Agent 框架，支持多种聊天平台，包括百度如流。

本指南将帮助你快速安装和配置 AnyClaw，并将其作为如流机器人使用。
...
这将把 `anyclaw` 安装到 `$GOPATH/bin` 或 `~/go/bin` 目录。
...
# 指定配置文件路径启动
./anyclaw start --config /path/to/config.json

# 以调试模式启动
./anyclaw start --log-level debug
```
...
1. 查看日志：`./anyclaw logs -f`
2. 使用调试模式：`./anyclaw start --log-level debug`
3. 阅读根目录 README.md
4. 查看 `docs/` 目录
