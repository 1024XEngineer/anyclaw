...
func getChannelStatusFromGateway(channelName string, timeout int) map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"channel": channelName,
		"status":  "unavailable",
		"message": "Gateway is not running. Start with 'anyclaw start' or 'anyclaw gateway run'",
	}
}
...
func outputChannelsJSON(activeChannels []ChannelInfo, allChannels []ChannelInfo) {
	...
	out := output{
		...
		Message: "Use 'anyclaw start' to start the gateway with configured channels",
	}
...
func outputChannelsText(activeChannels []ChannelInfo, allChannels []ChannelInfo) {
	...
	if gatewayOnline {
		...
		fmt.Println("Gateway: Offline (start with 'anyclaw start')")
	}
	...
	fmt.Println("\nTip:")
	fmt.Println("  1. Edit ~/.anyclaw/config.json to configure channels")
	fmt.Println("  2. Run 'anyclaw start' to start the agent with channels enabled")
	fmt.Println("  3. Use 'anyclaw channels status [name]' to check specific channel status")
}
