...
func TUICommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "tui",
		Short: "Open Terminal UI for anyclaw",
		Long:  `Open an interactive terminal UI for interacting with anyclaw agent.`,
		Run:   runTUI,
	}
...
func runTUI(cmd *cobra.Command, args []string) {
	...
	if runtime.GOOS == "windows" {
		...
		fmt.Fprintf(os.Stderr, "  anyclaw 'your prompt here'\n")
		os.Exit(1)
	}
	...
	fmt.Println("🐾 anyclaw Terminal UI")
	fmt.Println()
	...
	workspace := homeDir + "/.anyclaw/workspace"
	...
	sessionDir := homeDir + "/.anyclaw/sessions"
	...
	// 3. ~/.anyclaw/skills/ (用户全局目录)
	anyclawDir := homeDir + "/.anyclaw"
	globalSkillsDir := anyclawDir + "/skills"
	workspaceSkillsDir := workspace + "/skills"
	currentSkillsDir := "./skills"

	skillsLoader := agent.NewSkillsLoader(anyclawDir, []string{
