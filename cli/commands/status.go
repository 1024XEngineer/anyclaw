...
func StatusCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  `Display comprehensive status information about anyclaw sessions and gateway.`,
		Run:   runStatus,
	}
...
func runStatus(cmd *cobra.Command, args []string) {
	...
	status := &SystemStatus{
		SessionDir: filepath.Join(homeDir, ".anyclaw", "sessions"),
	}
...
func outputStatusText(status *SystemStatus) {
	...
	if status.Gateway.Online {
		...
		fmt.Printf("  Tip:     Start gateway with 'anyclaw gateway run'\n")
	}
	...
	if statusVerbose {
		...
		fmt.Printf("  Configuration: %s\n", filepath.Join(homeDir, ".anyclaw", "config.yaml"))
	}
