...
anyclaw 引入了先进的技能系统，允许用户通过编写 Markdown 文档 (`SKILL.md`) 来扩展 Agent 的能力。
...
anyclaw 按以下顺序查找配置文件（找到第一个即使用）：

1. `~/.anyclaw/config.json` (用户全局目录，**最高优先级**)
2. `./config.json` (当前目录)
3. 环境变量 `ANYCLAW_*` 前缀
...
| 顺序 | 路径 | 说明 |
|-----|------|------|
| 1 | `~/.anyclaw/skills/` | 用户全局目录（最低优先级） |
| 2 | `${WORKSPACE}/skills/` | 工作区目录 |
| 3 | `./skills/` (当前目录) | **最后加载，优先级最高** |

默认 `WORKSPACE` 为 `~/.anyclaw/workspace`。
...
# 单次执行 Agent
./anyclaw agent --message "你好，介绍一下你自己"
...
anyclaw 提供了丰富的命令行工具，主要命令包括：

### 基本命令

| 命令 | 描述 |
|-----|------|
| `anyclaw start` | 启动 Agent 服务 |
| `anyclaw tui` | 启动交互式终端界面 |
| `anyclaw agent --message <msg>` | 单次执行 Agent |
...
| 命令 | 描述 |
|-----|------|
| `anyclaw agents list` | 列出所有 agents |
| `anyclaw agents add` | 添加新 agent |
| `anyclaw agents delete <name>` | 删除 agent |

### Channel 管理

| 命令 | 描述 |
|-----|------|
| `anyclaw channels list` | 列出所有 channels |
| `anyclaw channels status` | 检查 channel 状态 |
| `anyclaw channels weixin login [account-id]` | 微信扫码登录 |
| `anyclaw channels weixin logout [account-id]` | 微信登出 |
| `anyclaw channels weixin status [account-id]` | 查看微信登录状态 |

### Gateway 管理

| 命令 | 描述 |
|-----|------|
| `anyclaw gateway run` | 运行 WebSocket Gateway |
| `anyclaw gateway install` | 安装为系统服务 |
| `anyclaw gateway status` | 查看 Gateway 状态 |

### Cron 定时任务

| 命令 | 描述 |
|-----|------|
| `anyclaw cron list` | 列出所有定时任务 |
| `anyclaw cron add` | 添加定时任务 |
| `anyclaw cron edit <id>` | 编辑定时任务 |
| `anyclaw cron run <id>` | 立即运行任务 |

### Browser 自动化

| 命令 | 描述 |
|-----|------|
| `anyclaw browser status` | 查看浏览器状态 |
| `anyclaw browser open <url>` | 打开 URL |
| `anyclaw browser screenshot` | 截图 |
| `anyclaw browser click <selector>` | 点击元素 |

### 其他命令

| 命令 | 描述 |
|-----|------|
| `anyclaw skills list` | 列出所有技能 |
| `anyclaw sessions list` | 列出所有会话 |
| `anyclaw memory status` | 查看记忆状态 |
| `anyclaw logs` | 查看日志 |
| `anyclaw health` | 健康检查 |
| `anyclaw status` | 状态查看 |
...
anyclaw 支持以下环境变量（前缀 `ANYCLAW_`）：

| 变量 | 描述 |
|-----|------|
| `ANYCLAW_CONFIG_PATH` | 配置文件路径 |
| `ANYCLAW_WORKSPACE` | 工作区目录 (默认: `~/.anyclaw/workspace`) |
| `ANTHROPIC_API_KEY` | Anthropic API Key |
| `OPENAI_API_KEY` | OpenAI API Key |
| `ANYCLAW_GATEWAY_URL` | Gateway WebSocket URL |
| `ANYCLAW_GATEWAY_TOKEN` | Gateway 认证 Token |

配置项可通过环境变量覆盖，例如：
- `ANYCLAW_AGENTS_DEFAULTS_MODEL` - 覆盖默认模型
- `ANYCLAW_TOOLS_SHELL_TIMEOUT` - 覆盖 Shell 工具超时时间
...
```bash
./anyclaw agent --message "测试" --thinking
./anyclaw logs -f
```
...
- Token 存储在 `~/.anyclaw/weixin/accounts/<account_id>.json`
