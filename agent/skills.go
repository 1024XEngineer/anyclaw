...
func NewSkillsLoader(workspace string, skillsDirs []string) *SkillsLoader {
	return &SkillsLoader{
		...
		autoInstall:    os.Getenv("ANYCLAW_SKILL_AUTO_INSTALL") == "true",
...
func (l *SkillsLoader) tryInstallBinary(skill *Skill, bin string) error {
	...
	case "node":
		...
		if nm := os.Getenv("ANYCLAW_NODE_MANAGER"); nm != "" {
			nodeManager = nm
		}
...
// ParseOpenClawMetadata 解析 OpenClaw/anyclaw 特定的元数据
