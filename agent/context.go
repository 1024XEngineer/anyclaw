...
func (b *ContextBuilder) buildSystemPromptWithSkills(skillsContent string, mode PromptMode) string {
	...
	if mode == PromptModeNone {
		return "You are a personal assistant running inside AnyClaw."
	}
...
func (b *ContextBuilder) buildIdentityAndTools() string {
	...
	coreToolSummaries := map[string]string{
		...
		"run_shell":              "Run shell commands. PROHIBITED: Never use 'crontab' commands for scheduled tasks - use the 'cron' tool instead (this is the ONLY way to manage scheduled tasks in anyclaw)",
		...
		"cron":                   "Manage anyclaw's built-in cron/scheduler service. This is the ONLY WAY to manage scheduled tasks. DO NOT use system 'crontab' commands. Supports: add (create), list/ls (view all), rm/remove (delete), enable, disable, run (execute immediately), status, runs (history)",
...
You are **AnyClaw**, a personal AI assistant running on the user's system.
...
User: "Search for information about anyclaw"
❌ "Here are some resources you can check..."
✅ (Calls: web_search with query "anyclaw") -> Shows search results
...
// buildCLIReference 构建 AnyClaw CLI 快速参考
func (b *ContextBuilder) buildCLIReference() string {
	return `## AnyClaw CLI Quick Reference

AnyClaw is controlled via subcommands. Do not invent commands.
...
If unsure, ask the user to run 'anyclaw help' (or 'anyclaw gateway --help') and paste the output.`
}
...
For AnyClaw behavior, commands, config, or architecture: consult the local documentation in README.md and docs/.
- When diagnosing issues, run 'anyclaw status' yourself when possible; only ask the user if you lack access.`
}
...
AnyClaw treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack.
