...
// GetAnyclawDir 获取 anyclaw 配置目录
func GetAnyclawDir() string {
	return filepath.Join(GetHomeDir(), ".anyclaw")
}

// GetConfigPath 获取配置文件路径
func GetConfigPath() string {
	return filepath.Join(GetAnyclawDir(), "config.json")
}

// EnsureBuiltinSkills 确保内置技能被复制到用户目录
// 支持增量复制：只复制缺失的技能，不会覆盖已存在的技能
func EnsureBuiltinSkills() error {
	anyclawDir := GetAnyclawDir()
	skillsDir := filepath.Join(anyclawDir, "skills")
...
func EnsureConfig() (bool, error) {
	...
	// 确保目录存在
	anyclawDir := GetAnyclawDir()
	if err := os.MkdirAll(anyclawDir, 0755); err != nil {
		return false, fmt.Errorf("failed to create anyclaw directory: %w", err)
	}
